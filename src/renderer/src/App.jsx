...
function TreeNode({ node, className }) {
  ...
  return (
    <>
      <div
        className={className ? className : "leftPanelTabElement"}
...
export default function App() {
  ...
  const filterLanguages = () => {
    return languages.filter((lang) => {
      if (
        lang.name.includes(modalInputText) ||
        lang.mode.includes(modalInputText) ||
        lang.extension.includes(modalInputText)
      )
        return lang;
    });
  };
        ...
        {showLeftPanel && (
          <>
            <Panel defaultSize={10} minSize={10} id={1} order={1}>
              ...
              <div className="leftPanelTab">
                <TreeNode
                  node={JSON.parse(localStorage.getItem("fileTreeData"))}
                  className="leftPanelTabTitle"
                />
              </div>
            </Panel>
            <PanelResizeHandle />
          ...
          <div className="filesTab">
            {files.map((element, i) => (
              <div
                ...
                {element.name}
                <div
                  className={`closeFileTab ${element.isChanged ? "changedFile" : ""}`}
                  onClick={(e) => {
                    handleCloseFile(i);
                    e.stopPropagation();
                  }}>
